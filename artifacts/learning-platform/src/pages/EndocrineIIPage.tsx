...
import {
  ALL_ENDOCRINE2_QUESTIONS,
  ENDOCRINE2_SECTIONS,
  type PQQuestion,
} from "@/data/endocrine2";
...
const SECTION_COLORS: Record<string, { text: string; bg: string; badge: string }> = {
  ...
  "Pediatrics":            { text: "text-indigo-700",  bg: "bg-indigo-50",  badge: "bg-indigo-100 text-indigo-700" },
  ...
  "Anatomy & Physiology":  { text: "text-orange-700",  bg: "bg-orange-50",  badge: "bg-orange-100 text-orange-700" },
};
...
export default function EndocrineIIPage() {
  ...
  const filteredQuestions = useMemo(() => {
    let qs = ALL_ENDOCRINE2_QUESTIONS;
  ...
  const maxCount = useMemo(() => {
    if (practiceConfig.section === "All Sections") return 273;
    const sec = ENDOCRINE2_SECTIONS.find((s) => s.name === practiceConfig.section);
    return sec?.count ?? 273;
  ...
  function startPractice() {
    const pool =
      practiceConfig.section === "All Sections"
        ? ALL_ENDOCRINE2_QUESTIONS
        : ALL_ENDOCRINE2_QUESTIONS.filter(
            (q) => q.section === practiceConfig.section
          );
    const shuffled = [...pool].sort(() => Math.random() - 0.5);
    const selected = shuffled.slice(0, practiceConfig.count);
    localStorage.setItem("qym_e2_questions", JSON.stringify(selected));
    localStorage.setItem("qym_e2_minutes", String(practiceConfig.minutes));
    navigate("/past-questions/level-400/endocrine-ii/practice");
  }
          ...
          <div className="rounded-2xl border border-border bg-card p-8 shadow-sm">
            <div className="mb-6 flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">ENDOCRINE II — 273 Questions</p>
              </div>
            </div>
            ...
            <div className="space-y-6">
              <div>
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    const cap =
                      sec === "All Sections"
                        ? 273
                        : (ENDOCRINE2_SECTIONS.find((s) => s.name === sec)?.count ?? 273);
                    setPracticeConfig((p) => ({
                      ...p,
                      section: sec,
                ...
                >
                  <option value="All Sections">All Sections (273 questions)</option>
                  {ENDOCRINE2_SECTIONS.map((s) => (
    ...
    <DashboardLayout>
      <div className="mx-auto max-w-4xl px-4 py-8">
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <div className="mb-1 flex items-center gap-2 text-xs text-muted-foreground">
                ...
                <span className="font-semibold text-foreground">ENDOCRINE II</span>
              </div>
              <h1 className="text-3xl font-bold text-foreground">ENDOCRINE II</h1>
        ...
        <div className="mb-8 grid grid-cols-2 gap-3 sm:grid-cols-4">
          {[
            { label: "Total Questions", value: "273" },
        ...
        <div className="mb-6">
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            {ENDOCRINE2_SECTIONS.map((sec) => {
              const col = SECTION_COLORS[sec.name];
              return (
                <button
                  key={sec.name}
                  onClick={() => {
