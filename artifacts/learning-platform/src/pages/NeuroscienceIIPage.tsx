...
import {
  ALL_NEUROSCIENCE2_QUESTIONS,
  NEUROSCIENCE2_SECTIONS,
  type PQQuestion,
} from "@/data/neuroscience2";
...
const SECTION_COLORS: Record<string, { text: string; bg: string; badge: string }> = {
  ...
  "Surgery":             { text: "text-indigo-700",  bg: "bg-indigo-50",  badge: "bg-indigo-100 text-indigo-700" },
  "Pathology":           { text: "text-violet-700",  bg: "bg-violet-50",  badge: "bg-violet-100 text-violet-700" },
  "Pediatrics":          { text: "text-purple-700",  bg: "bg-purple-50",  badge: "bg-purple-100 text-purple-700" },
  "Pharmacology":        { text: "text-fuchsia-700", bg: "bg-fuchsia-50", badge: "bg-fuchsia-100 text-fuchsia-700" },
  "Radiology":           { text: "text-rose-700",    bg: "bg-rose-50",    badge: "bg-rose-100 text-rose-700" },
  "Chemical Pathology":  { text: "text-orange-700",  bg: "bg-orange-50",  badge: "bg-orange-100 text-orange-700" },
  "Others":              { text: "text-teal-700",    bg: "bg-teal-50",    badge: "bg-teal-100 text-teal-700" },
};

const TOTAL = ALL_NEUROSCIENCE2_QUESTIONS.length;
...
export default function NeuroscienceIIPage() {
  ...
  const filteredQuestions = useMemo(() => {
    let qs = ALL_NEUROSCIENCE2_QUESTIONS;
  ...
  const maxCount = useMemo(() => {
    ...
    const sec = NEUROSCIENCE2_SECTIONS.find((s) => s.name === practiceConfig.section);
  ...
  function startPractice() {
    const pool =
      practiceConfig.section === "All Sections"
        ? ALL_NEUROSCIENCE2_QUESTIONS
        : ALL_NEUROSCIENCE2_QUESTIONS.filter(
            (q) => q.section === practiceConfig.section
          );
    const shuffled = [...pool].sort(() => Math.random() - 0.5);
    const selected = shuffled.slice(0, practiceConfig.count);
    localStorage.setItem("qym_neuro2_questions", JSON.stringify(selected));
    localStorage.setItem("qym_neuro2_minutes", String(practiceConfig.minutes));
    navigate("/past-questions/level-400/neuroscience-ii/practice");
  }
          ...
          <div className="rounded-2xl border border-border bg-card p-8 shadow-sm">
            <div className="mb-6 flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">NEUROSCIENCE II — {TOTAL} Questions</p>
              </div>
            </div>
            ...
            <div className="space-y-6">
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    const cap =
                      sec === "All Sections"
                        ? TOTAL
                        : (NEUROSCIENCE2_SECTIONS.find((s) => s.name === sec)?.count ?? TOTAL);
                    setPracticeConfig((p) => ({
                      ...p,
                      section: sec,
              ...
              <button
                ...
                className="w-full rounded-xl bg-purple-600 py-3 text-base font-bold text-white shadow-sm hover:bg-purple-700 transition-colors"
              >
                Start Practice Exam
              </button>
            </div>
      ...
      <div className="mx-auto max-w-4xl px-4 py-8">
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <div className="mb-1 flex items-center gap-2 text-xs text-muted-foreground">
                ...
                <span className="font-semibold text-foreground">NEUROSCIENCE II</span>
              </div>
              <h1 className="text-3xl font-bold text-foreground">NEUROSCIENCE II</h1>
              <p className="mt-1 text-sm text-muted-foreground">
                Past Questions Compilation 2026 · {TOTAL} Questions · 8 Sections
              </p>
            </div>
            <button
              onClick={() => setView("practice-config")}
              className="inline-flex items-center gap-2 rounded-xl bg-purple-600 px-5 py-3 text-sm font-bold text-white shadow-sm hover:bg-purple-700 transition-colors"
            >
        ...
        <div className="mb-6">
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            {NEUROSCIENCE2_SECTIONS.map((sec) => {
              return (
                <button
                  key={sec.name}
                  onClick={() => {
