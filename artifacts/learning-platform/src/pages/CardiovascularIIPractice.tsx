...
import { type PQQuestion } from "@/data/cardiovascular2";
...
const SECTION_COLORS: Record<string, string> = {
  ...
  "Anatomy & Radiology": "bg-orange-100 text-orange-700",
};
...
export default function CardiovascularIIPractice() {
  const [, navigate] = useLocation();

  const [questions] = useState<PQQuestion[]>(() => {
    try {
      const stored = localStorage.getItem("qym_cv2_questions");
  ...
  const [minutes] = useState(() => {
    try {
      return Number(localStorage.getItem("qym_cv2_minutes") ?? "0");
    } catch {
      return 0;
    }
  });
                  ...
                  <button
                    ...
                    className={`flex w-full items-start gap-3 rounded-xl border-2 p-4 text-left transition-all ${
                      selected
                        ...
                        : "border-border bg-card hover:border-red-200 hover:bg-red-50/30"
                    }`}
                  >
                    <span
            ...
            <div className="mt-8 flex items-center justify-between">
              ...
              {current < questions.length - 1 ? (
                <button
                  ...
                  className="flex items-center gap-2 rounded-lg bg-red-600 px-4 py-2.5 text-sm font-bold text-white hover:bg-red-700 transition-colors"
                >
                  Next
